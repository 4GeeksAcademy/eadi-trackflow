...
export default function ProfilePage() {
  ...
  return <main className="authPage"><form className="authCard" onSubmit={submit}><h1>Mi perfil</h1><p>{user.email}</p><label>Nombre<input name="name" defaultValue={user.profile?.name ?? ""} /></label><label>Teléfono<input name="phone" defaultValue={user.profile?.phone ?? ""} /></label><label>Dirección<input name="address" defaultValue={user.profile?.address ?? ""} /></label>{message && <p>{message}</p>}<button>Guardar cambios</button><button type="button" className="logoutButton" onClick={() => { clearToken(); router.replace("/login"); }}>Cerrar sesión</button></form></main>;
}